...
```text
Suffix: %misttags_suffix_or_none%
```

Keep nametags/tablist on `%misttags_display_suffix%` so players without a suffix stay clean instead of showing `None` above their head.
